...
<form action="{{route('dishstorepost')}}" class="dishform" method="POST" enctype="multipart/form-data"   >
  @csrf
  <div class="form-item">
   <input type="text" name="name" placeholder="Название" required="">
  </div>
  <div class="form-item">
   <input type="number" name="price" placeholder="Цена в рублях" required="">
  </div>
  <div class="form-item">
  <input type="file" name="image">
   </div>
   <h3 style="margin: 0; padding: 0; margin-top: 25px;">Продукты</h3>
  <div class="form-item">
  ...
  <div class="form-item">
  @foreach($components as $value)
     <div class="hide-input" data-id="{{$value->id}}" >
        <p>{{$value->name}} </p>
        <input  name="component-{{$value->id}}" placeholder="грамм"  value="">
...
<h3 style="margin: 0; padding: 0; margin-top: 25px;">Блюда</h3>
  ...
  <div class="form-item">
  @foreach($dishes as $value)
     <div class="hide-input-dish" data-id="{{$value->id}}" >
        <p>{{$value->name}} </p>
        <input  name="dish-{{$value->id}}" placeholder="грамм"  value="">
        <a class="delete" data-id="{{$value->id}}"  style="color: red"  > Удалить</a>
     </div>
  @endforeach
</div>
<p> <input type="checkbox" name="interim"> Промежуточные блюдо </p>
